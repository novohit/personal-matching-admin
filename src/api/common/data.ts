...
// 获取用户访问趋势
export const getUserVisitTrendData = (type: string, beginDate: string, endDate: string | null = null) => {
  if (type === 'daily') {
    return http.request<ResponseData<UserVisitTrendResponse>>("get", "/wechat/analysis/trend/daily", { params: { date: beginDate } });
  } else if (type === 'weekly') {
...
export const getUserSummaryData = (date: string) => {
  return http.request<ResponseData<UserSummaryItem>>("get", "/wechat/analysis/summary/daily", { params: { date } });
};

// 用户留存趋势
interface UserRetainTrendItem {
  ref_date: string;
  visit_uv_new: {
    key: number;
    value: number;
...
// 获取用户留存趋势
export const getUserRetainTrendData = (type: string, beginDate: string, endDate: string | null = null) => {
  if (type === 'daily') {
    return http.request<ResponseData<UserRetainTrendResponse>>("get", `/wechat/analysis/retain/daily`, { params: { date: beginDate } });
  } else if (type === 'weekly') {
    return http.request<ResponseData<UserRetainTrendResponse>>("get", `/wechat/analysis/retain/weekly`, { params: { beginDate, endDate } });
  } else if (type === 'monthly') {
    return http.request<ResponseData<UserRetainTrendResponse>>("get", `/wechat/analysis/retain/monthly`, { params: { beginDate, endDate } });
  }
};